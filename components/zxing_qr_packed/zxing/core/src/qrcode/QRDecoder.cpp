...
// QRTransfer modification: add decode telemetry only (codec mode mask); decoding behavior unchanged.
...
DecoderResult DecodeBitStream(ByteArray&& bytes, const Version& version, ErrorCorrectionLevel ecLevel)
{
	...
	uint32_t codecModeMask = 0; // QRTransfer telemetry: observed QR segment modes
	...
	try
	{
		while(!IsEndOfStream(bits, version)) {
			...
			codecModeMask |= (1u << static_cast<unsigned>(mode));
...
//					throw FormatError("GS1 Indicator (FNC1 in first position) at illegal position");
	...
	return DecoderResult(std::move(result))
		...
		.addExtra("CodecModeMask", codecModeMask);
}
