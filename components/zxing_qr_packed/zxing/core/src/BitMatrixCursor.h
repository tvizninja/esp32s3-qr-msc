...
* Modified for QRTransfer in 2026: added a packed-view fallback for edge counting while retaining the stock byte fast path.
*/
...
public:
	FastEdgeToEdgeCounter(const BitMatrixCursorI& cur) : img(cur.img), p(cur.p), d(cur.d), packed(cur.img->isPackedView())
	{
		assert(cur.isIn());
		if (!packed) {
			byteStride = cur.d.y * cur.img->width() + cur.d.x;
			bytePtr = cur.img->row(cur.p.y).begin() + cur.p.x;
		}
	...
	int stepToNextEdge(int range)
	{
		...
		if (packed) {
			...
			} while (bytePtr[steps * byteStride] == bytePtr[0]);
			bytePtr += steps * byteStride;
		}
