...
* Modified for QRTransfer in 2026: added a non-owning read-only MSB-first packed 1bpp input view.
*/
...
class BitMatrix
{
	...
	const uint8_t* _packedView = nullptr;
	int _packedStride = 0;
...
public:
	...
	// Construct a non-owning 1bpp MSB-first packed view. The caller owns `data`
	// and must keep it alive for the lifetime of the BitMatrix. Read-only.
	static BitMatrix FromPackedView(int width, int height, const uint8_t* data, int strideBytes = 0)
	{
		if (width <= 0 || height <= 0 || data == nullptr)
			throw std::invalid_argument("Invalid packed BitMatrix view");
		BitMatrix res;
		res._width = width;
		res._height = height;
		res._packedView = data;
		res._packedStride = strideBytes ? strideBytes : (width + 7) / 8;
		if (res._packedStride < (width + 7) / 8)
			throw std::invalid_argument("Packed BitMatrix stride too small");
		return res;
	}

	bool isPackedView() const noexcept { return _packedView != nullptr; }
	...
	{
		if (_packedView) {
			auto b = _packedView[y * _packedStride + (x >> 3)];
			return (b >> (7 - (x & 7))) & 1;
		}
	...
	{
		if (_packedView)
			throw std::logic_error("Attempt to modify read-only packed BitMatrix view");
	...
	*/
	...
	bool empty() const { return _packedView == nullptr && _bits.empty(); }
