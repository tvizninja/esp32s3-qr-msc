...
* Modified for QRTransfer in 2026: added packed-view pattern-row access while retaining the stock byte fast path.
*/
...
void GetPatternRow(const BitMatrix& matrix, int r, std::vector<uint16_t>& pr, bool transpose)
{
	...
	// Packed input view: build the same white/black run-length row through
	// the public accessor. Finder detection consumes run lengths, not raw bytes.
	const int n = transpose ? matrix.height() : matrix.width();
	pr.clear();
	pr.reserve(64);
	if (n <= 0)
		return;
	auto at = [&](int i) { return transpose ? matrix.get(r, i) : matrix.get(i, r); };
	bool last = at(0);
	if (last)
		pr.push_back(0); // PatternRow always starts with a white run.
	uint16_t run = 1;
	for (int i = 1; i < n; ++i) {
		bool cur = at(i);
		if (cur == last) {
	...
	pr.push_back(run);
	if (last)
		pr.push_back(0); // PatternRow always ends with a white run.
}
